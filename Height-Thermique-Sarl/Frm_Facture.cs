...
namespace Height_Thermique_Sarl
{
    public partial class Frm_Facture : Form
    {
        ...
        private void Frm_Facture_Load(object sender, EventArgs e)
        {
            ...
            // Devis table 
            dapDevis = new SqlDataAdapter("select * from Facture", cnx);
            dapDevis.Fill(ds, "Facture");
        ...
        private void supprimerLigneSelectionnéToolStripMenuItem_Click(object sender, EventArgs e)
        {
            dgvallProdutcs.Rows.RemoveAt(dgvallProdutcs.CurrentRow.Index);
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (cmbCivilite.Text == "" || txtCltName.Text == "" || txtNumDevis.Text == "" || txtSeller.Text == "")
                {
                    MessageBox.Show("Merci de remplir les champs !");
                    return;
                }

                ligne = ds.Tables["Facture"].NewRow();
                ligne[1] = txtNumDevis.Text;
                ligne[2] = txtSeller.Text;
                ligne[3] = dtpDevis.Text;
                ligne[4] = cmbCivilite.Text;
                ligne[5] = txtCltName.Text;

                ds.Tables["Facture"].Rows.Add(ligne);
                cb = new SqlCommandBuilder(dapDevis);
                dapDevis.Update(ds, "Facture");
                ...
                for (int i = 0; i < dgvallProdutcs.Rows.Count - 1; i++)
                {
                    ligne = ds.Tables["ProduitsFacture"].NewRow();
                    ...
                    ds.Tables["ProduitsFacture"].Rows.Add(ligne);
