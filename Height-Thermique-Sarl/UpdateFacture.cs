...
{
    public partial class UpdateFacture : Form
    {
        ...
        private void UpdateFacture_Load(object sender, EventArgs e)
        {
            CreateDataTable();
            resizeDGV();

            if (ds.Tables["ProduitsFacture"] != null)
                ds.Tables["ProduitsFacture"].Clear();

            dapProduits = new SqlDataAdapter("select * from ProduitsFacture", cnx);
            dapProduits.Fill(ds, "ProduitsFacture");
        }
        ...
        private void btnRechercher_Click(object sender, EventArgs e)
        {
            try
            {
                if (ds.Tables["UpdateFacture"] != null)
                    ds.Tables["UpdateFacture"].Clear();

                dap = new SqlDataAdapter("select NumProduits, Designations, Unite, Quantite, PrixUnitaire, Total from ProduitsFacture where NumFacture = '" + txtSearch.Text + "'", cnx);
                dap.Fill(ds, "UpdateFacture");
                dgvUpdate.DataSource = ds.Tables["UpdateFacture"];
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                //Delete Old Products
                cmd.Connection = cnx;
                cmd.CommandText = "delete  from ProduitsFacture where NumFacture ='" + txtSearch.Text + "'";
                cnx.Open();
                cmd.ExecuteNonQuery();
                cnx.Close();
                ...
                for (int i = 0; i < dgvUpdate.Rows.Count; i++)
                {
                    lgn = ds.Tables["ProduitsFacture"].NewRow();
                    ...
                    ds.Tables["ProduitsFacture"].Rows.Add(lgn);

                }

                cb = new SqlCommandBuilder(dapProduits);
                dapProduits.Update(ds, "ProduitsFacture");
        ...
        private void supprimerUneLigneSelectionneToolStripMenuItem_Click(object sender, EventArgs e)
        {
            dgvUpdate.Rows.RemoveAt(dgvUpdate.CurrentRow.Index);
        }
