...
{
    public partial class Update_Situation : Form
    {
        public Update_Situation()
        {
            InitializeComponent();
        }
        ...
        private void Update_Situation_Load(object sender, EventArgs e)
        {
            CreateDataTable();
            resizeDGV();

            if (ds.Tables["ProduitSTT"] != null)
                ds.Tables["ProduitSTT"].Clear();
        ...
        private void btnRechercher_Click(object sender, EventArgs e)
        {
            try
            {
                if (ds.Tables["UpdateSTT"] != null)
                    ds.Tables["UpdateSTT"].Clear();

                dap = new SqlDataAdapter("Select NumProduits, Designations, Unite, Quantite, PrixUnitaire, Total from ProduitSTT where NumSTT = '" + txtSearch.Text + "'", cnx);
                dap.Fill(ds, "UpdateSTT");
                dgvUpdate.DataSource = ds.Tables["UpdateSTT"];
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                //Delete Old Products
                cmd.Connection = cnx;
                cmd.CommandText = "delete  from ProduitSTT where NumSTT ='" + txtSearch.Text + "'";
                cnx.Open();
                cmd.ExecuteNonQuery();
                cnx.Close();
                ...
                for (int i = 0; i < dgvUpdate.Rows.Count; i++)
                {
                    lgn = ds.Tables["ProduitSTT"].NewRow();
                    ...
                    ds.Tables["ProduitSTT"].Rows.Add(lgn);
